...
def main_worker(args):
    ...
    ckpt_begin = torch.load(args.begin, map_location='cpu')
    mkg = model.load_state_dict(ckpt_begin['state_dict'], strict=False)
    print("Unloaded parameters:")
    print(mkg)
    tune_list = ['module.base.0.pos_embed_1', 'module.base.0.pattern_1.weight', \
                 'module.base.0.matrix_1.weight', 'module.classifier.weight']
    for key, value in model.named_parameters():
        if key not in tune_list:
            value.requires_grad = False
        else:
            print("We optimize %s!"%key)
...
if __name__ == '__main__':
    ...
    # training configs
    parser.add_argument('--begin', type=str, default='', metavar='PATH')
